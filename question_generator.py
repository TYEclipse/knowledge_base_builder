...
from config import MAX_RESEARCH_SUMMARY_PROMPT_CHARS, PHASE2_MAX_TOKENS, QUESTIONS_PER_LEVEL
...
class QuestionGenerator:
    ...
    def _build_prompts(self, level_en: str, level_cn: str, research_summary: str) -> Tuple[str, str]:
        system_prompt = (
    ...
    def generate(self, research_summary: str) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """生成三级问题。"""
        levels = [
            ("beginner", "初学者"),
            ("intermediate", "中级学习者"),
        ...
        for level_en, level_cn in levels:
            system_prompt, user_prompt = self._build_prompts(level_en, level_cn, research_summary)
            try:
                ...
                while len(questions) < QUESTIONS_PER_LEVEL:
                    questions.append(f"[{self.topic} {level_cn}] 补充问题 {len(questions) + 1}")
                questions = questions[:QUESTIONS_PER_LEVEL]

                phase2_records.append(JsonlRecordFactory.phase2(level=payload.get("level", level_en), topic=payload.get("topic", self.topic), questions=questions))

                for q in questions:
                    qid += 1
                    item = QuestionItem(id=qid, level=str(payload.get("level", level_en)), question=str(q))
                    all_questions.append(item.model_dump())
            except Exception as exc:
                self.logger.warning("问题生成失败(level=%s): %s", level_en, exc)
                fallback_questions = [
                    f"[{self.topic} {level_cn}] 默认问题 {i + 1}（生成失败降级）" for i in range(QUESTIONS_PER_LEVEL)
                ]
                phase2_records.append(JsonlRecordFactory.phase2(level=level_en, topic=self.topic, questions=fallback_questions))
