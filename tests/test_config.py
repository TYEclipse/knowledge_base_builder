...
def test_build_settings_from_args(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("KIMI_TOOL_DEBUG", "true")
    monkeypatch.setenv("KIMI_TOOL_DEBUG_MAX_CHARS", "1200")
    ...
    assert settings.tool_debug_max_chars == 1200


def test_build_settings_tool_debug_invalid_max_chars_uses_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MOONSHOT_API_KEY", "k-test")
    monkeypatch.setenv("KIMI_TOOL_DEBUG", "1")
    monkeypatch.setenv("KIMI_TOOL_DEBUG_MAX_CHARS", "not-a-number")
